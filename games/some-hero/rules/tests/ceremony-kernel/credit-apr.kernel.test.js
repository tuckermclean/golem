// Mirror of games/some-hero/ceremony/credit-apr.ceremony.test.js against
// rules/ instead of legacy/src. S2a design spec DoD: "credit-apr — fully
// (the one respawnAtGuild-on-blankGame test runs on a plain object, no
// real zone — safe here)." All 12 ceremony tests covered, 0 deferred.
...
import { respawnAtGuild } from "../../meta.js";
import { blankGame, spyFx } from "./fixtures.js";

// ceremony/credit-apr.ceremony.test.js:17-21
test("@ceremony-kernel score constants: MIN 300, MAX 850, START 650", () => {
  assert.equal(SCORE_MIN, 300);
  assert.equal(SCORE_MAX, 850);
  assert.equal(SCORE_START, 650);
});

// ceremony/credit-apr.ceremony.test.js:23-36
test("@ceremony-kernel APR tiers are exact: .0999 / .2499 / .3999 / .9999, tier names PREFERRED/STANDARD/SUBPRIME/ADVENTUROUS", () => {
...
// ceremony/credit-apr.ceremony.test.js:38-49
test("@ceremony-kernel creditLimit: no income = 0; else 4x income scaled by tier multiplier (1.5x / 1x / .5x / .25x)", () => {
...
// ceremony/credit-apr.ceremony.test.js:51-62
test("@ceremony-kernel canBorrow declines with exact reasons: no income / limit / score(<500) / delinquent(missed>=2)", () => {
  const game = blankGame();
...
// ceremony/credit-apr.ceremony.test.js:64-72
test("@ceremony-kernel borrow() adds principal; the debt credential is set and stays set after payDown", () => {
  const game = blankGame();
  game.meta.income = 15;
  borrow(game.meta, 60);
...
// ceremony/credit-apr.ceremony.test.js:74-82
test("@ceremony-kernel interest compounds per excursion, one excursion == one month (APR/12, rounded up)", () => {
  const game = blankGame();
...
// ceremony/credit-apr.ceremony.test.js:84-89
test("@ceremony-kernel minPayment: 1/8 of balance + 2, full balance due at or below 8g", () => {
...
// ceremony/credit-apr.ceremony.test.js:91-113
test("@ceremony-kernel death-payment ladder: on-time +10, short -60 and missed++, clearing +25 and forgiveness", () => {
  const game = blankGame();
...
// ceremony/credit-apr.ceremony.test.js:115-124
test("@ceremony-kernel score clamps to [300, 850]", () => {
  const game = blankGame();
  const c = game.meta.credit;
...
// ceremony/credit-apr.ceremony.test.js:126-143
test("@ceremony-kernel garnishment rides the real death path, itemized, after the resurrection deductible", () => {
...
// ceremony/credit-apr.ceremony.test.js:145-161
test("@ceremony-kernel Truth in Lending disclosure: exact line count and pinned strings, including house-spelling notarization", () => {
  const game = blankGame();
  game.meta.income = 15;
  borrow(game.meta, 60);
...
// ceremony/credit-apr.ceremony.test.js:163-166
test("@ceremony-kernel BITE: APR at score 650 is .2499, not .25 — a rounder-looking wrong constant fails", () => {
  assert.equal(aprFor(650), .2499);
